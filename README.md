...
DiskRead is a program that allows you to read raw chunks of a disk or a file. It aims to replace `xxd` or `hexdump` present on Linux, but for Windows. Furthermore, the low-level WinAPI functions enable DiskRead to DiskRead access any file, including boot sectors and disks, both for reading and writing.
